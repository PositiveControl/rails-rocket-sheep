...
| Rule | Tokens | Covers |
|---|---|---|
| [api-auth](api-auth.md) | 720 | Doorkeeper, coarse scopes, server-side revocation |
...
| [async-202](async-202.md) | 600 | 202, a `Location`, and a real status resource |
...
| [client-contract](client-contract.md) | 740 | What the client may rely on, and what it must not |
| [controllers](controllers.md) | 630 | Seven actions, new verb = new resource, thin actions |
| [cors](cors.md) | 580 | Named origins, and never a wildcard with credentials |
| [current-attributes](current-attributes.md) | 230 | Two acceptable uses, and why |
| [cursor-pagination](cursor-pagination.md) | 860 | Cursor first, `(created_at, id)`, never an array |
...
| [error-envelope](error-envelope.md) | 830 | RFC 9457 problem documents, and a machine-readable `type` |
...
| [idempotency](idempotency.md) | 740 | A key on every write that costs money |
...
| [serialization](serialization.md) | 680 | One plain object per resource, explicit fields, nothing else |
...
| [status-codes](status-codes.md) | 890 | Which status, and the outcome that is neither success nor failure |
| [testing](testing.md) | 1010 | Minitest + fixtures, which layer tests what, VCR |
| [write-path](write-path.md) | 320 | Layer order end to end |

**Total corpus:** ~23,420 tokens across 40 rules — but nobody reads it whole.
...
## Still carrying their web flavour

These ship to both modes and their bodies were written for an HTML app. They are right
about the layering and wrong about the response half until the API pass rewrites them:
`controllers`, `exception-boundary`, `pattern-budget`, `policy-objects`, `rate-limiting`, `service-objects`, `testing`, `write-path`.
