...
**Query ledger**
`db/queries.yml`: every distinct SQL shape the suite emits from `app/`, each with a
one-line `review:` naming the index it uses or starting `no index:` with the reason.
Generated by `bin/rails db:queries`, reviewed by a person, checked in CI. It records
that a query was looked at, not that its plan is good.
*Avoid*: "query log" — a log is every query that ran; the ledger is every shape,
once, with a decision attached.
