...
which needs a running database server, a bundle install, and a few minutes. There
is no gem, no runtime, and nothing importable to unit-test: `template.rb` is a
Thor script whose observable behaviour is a directory tree.
...
Generation breaks twice from the same cause, or a CI runner with a database is
cheap enough to run one probe generation per push (two, since
[ADR 0007](0007-database-family-is-chosen-at-generation.md) — half the branches
are otherwise never executed). A single smoke job that generates an app and runs
`bin/test` would retire most of this ADR.
